...
export default async function handler(req){
  ...
  let payload={}; try{payload=await req.json()}catch{}; payload.ts=payload.ts||new Date().toISOString();
  ...
  if(hook){ try{
    ...
    }catch(e){ return new Response(JSON.stringify({ok:false,error:'forward_failed'}),{status:500,headers:{'content-type':'application/json'}}); } }
